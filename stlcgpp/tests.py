...
def test_always(signal, interval, verbose=True, **kwargs):
    def true_robustness_trace(signal, interval, **kwargs):
        T = len(signal)
        if (interval is None):
            return torch.stack([minish(signal[i:], dim=0, keepdim=False, **kwargs) for i in range(len(signal))])
        else:
            if interval[1] == torch.inf:
                large_number = 1E9
                start = interval[0]
                signal_padded = torch.cat([signal, torch.ones(T-1) * large_number, torch.ones(start) * -large_number])
                return torch.stack([minish(signal_padded[i+start:i+start+T], dim=0, keepdim=False, **kwargs) for i in range(len(signal))])
            else:
                large_number = 1E9
                signal_padded = torch.cat([signal, torch.ones(interval[1]+1) * -large_number])
                return torch.stack([minish(signal_padded[i:][interval[0]:interval[1]+1], dim=0, keepdim=False, **kwargs) for i in range(len(signal))])
...
def test_eventually(signal, interval, verbose=True, **kwargs):
    def true_robustness_trace(signal, interval, **kwargs):
        T = len(signal)
        if (interval is None):
            return torch.stack([maxish(signal[i:], dim=0, keepdim=False, **kwargs) for i in range(len(signal))])
        else:
            if interval[1] == torch.inf:
                large_number = 1E9
                start = interval[0]
                signal_padded = torch.cat([signal, torch.ones(T) * -large_number])
                return torch.stack([maxish(signal_padded[i+start:i+T], dim=0, keepdim=False, **kwargs) for i in range(len(signal))])
            else:
                large_number = 1E9
                signal_padded = torch.cat([signal, torch.ones(interval[1]+1) * -large_number])
                return torch.stack([maxish(signal_padded[i:][interval[0]:interval[1]+1], dim=0, keepdim=False, **kwargs) for i in range(len(signal))])
...
def test_until(signal, interval, verbose=True, **kwargs):
    ...
    def true_robustness_trace(signal, interval, **kwargs):
        ...
        return torch.stack([maxish(torch.stack([minish(torch.stack([minish(signal1_padded[i:][:t+1], dim=0, keepdim=False, **kwargs), signal2_padded[i:][t]]), dim=0, keepdim=False, **kwargs) for t in range(interval[0],interval[-1]+1)]), dim=0, keepdim=False, **kwargs) for i in range(T)])
...
def test_always_mask_recurrent(signal, interval, verbose=True, **kwargs):
    ...
    rob_correct = torch.isclose(mask.robustness(signal, **kwargs), rec.robustness(signal_flip, **kwargs))
    trace_correct = torch.all(torch.isclose(mask(signal, **kwargs), rec(signal_flip, **kwargs).flip(0)))
    grad_correct = torch.all(torch.isclose(torch.func.grad(mask.robustness)(signal, **kwargs), torch.func.grad(rec.robustness)(signal_flip, **kwargs).flip(0)))
...
def test_all_settings(test_func, verbose=True, T=10):
    ...
    interval_list = [None, [0, T//3], [T//4, T//2], [0, torch.inf], [T//4, torch.inf]]
